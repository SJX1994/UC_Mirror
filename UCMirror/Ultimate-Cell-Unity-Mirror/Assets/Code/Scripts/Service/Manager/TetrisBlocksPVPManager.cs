...
public class TetrisBlocksPVPManager : MonoBehaviour
{
    // 将配置高度导入方块中
    public int _height;
    ...
    // 待创建Unit
    private List<UnitInfoClass> BlocksReadyUnit = new();
    ...
    private void Update()
    {
        if (cencel)
        {
            if (Input.GetMouseButtonDown(0))
            {
                ...
                var tetrisId = _grid[(int)((hit.point.x + 13.75) / 1.5 + 0.5), (int)((hit.point.z + 8.6) / 1.5 + 0.5)];
                ...
                if (!_AllTetrisDic.ContainsKey(int.Parse(tetrisId)))
                {
                    cencel = false;
                ...
                Dictionary<int, TetrisClass> delete = new();

                var tetrisInfo = _AllTetrisDic[int.Parse(tetrisId)];

                if (tetrisInfo.unitType == EventType.UnitType.Hero)
                {
                    delete = FindObjectOfType<HeroManager>().FindHeroBlocks(int.Parse(tetrisId), true);
                }
                else
                {
                    delete.Add(int.Parse(tetrisId), _AllTetrisDic[int.Parse(tetrisId)]);

                }

                // 删除显示砖块
                var deleteInfo = OnCreateUnitInfo(delete, false);
                ...
                AudioSystemManager.Instance.PlaySound("Click_Twice");
            }
        }
    }
    ...
    public void CreateUnit()
    {
        ...
        CommunicationManager.OnCellUnitCreate(BlocksReadyUnit);
    ...
    {
        // 计算砖块合成
        CountGridClass countGrid = OnCreateUnitButtonClick();

        // 删除显示砖块
        var deleteInfo = OnCreateUnitInfo(countGrid.tetrisDelete, false);
        ...
        // 新建Unit
        var createInfo = OnCreateUnitInfo(countGrid.tetrisUpdate, false);
    ...
    /// </summary>
    /// <param name="info"></param>
    public void BlocksReadyUnitCreate(Dictionary<int, TetrisClass> info)
    {
